...
function createCustomer(id: number, nom: string, email: string): Customer {
    return {
        id: id,
        name: nom,
        email: email
...
function createProduct(id: number, name: string, price: number, stock: number): Product{
    return {
        id: id,
        name: name,
        price: price,
...
function createOrderItem(product: Product, quantity: number): OrderItem {
    return {
        product: product,
        quantity: quantity
    }
...
let calculOrder2: number = calculateTotal(order2)
